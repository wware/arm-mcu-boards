...
# Working on 64-bit Ubuntu 12.04 Precise in a VirtualBox on a Mac
...
do_cmd('sudo apt-get install libgmp3-dev libmpfr-dev texinfo libncurses5-dev ' +
       'aptitude build-essential git-core wget screen dfu-util openocd ' +
...
class Package:
    ...
    def unpack_source(self):
        ...
        if self.url.endswith('.gz'):
            ...
            cmdopts = 'jxvf'
        cmd = '(cd %s; tar %s %s -C %s)' % (srcdir, cmdopts, path, srcdir)
        do_cmd(cmd)
    ...
    def build_phase1(self, arch):
        ...
        cmd = '(cd %s; %s/configure %s)' % (self.blddir,
                                            ...
                                            opts)
        do_cmd(cmd)
        self.build_phase1_finish()

    def build_phase1_finish(self):
...
class Architecture:
    def __init__(self, name):
        self.name = name
        self.prefix = os.path.join(os.environ['HOME'], 'arm')
        self.gccopts = '''--target=TARGET-OBJTYPE \
            --prefix=PREFIX \
            --with-gmp=/usr/lib/x86_64-linux-gnu \
            --with-mpfr=/usr/lib/x86_64-linux-gnu \
            --enable-interwork \
            --enable-multilib \
            --with-float=soft \
            --nfp \
...
mkdir(arch.prefix)
do_cmd('export PATH=$PATH:%s/bin' % arch.prefix)


map(lambda pkg: pkg.procure_source_tarball(),
    packages)
